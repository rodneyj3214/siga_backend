    ...
    public function up()
    {
        Schema::connection('pgsql-community')->create('projects', function (Blueprint $table) {
           ...
           // $table->foreignId('academi_period_id')->connstrained('vinculacion.academi_periods'); y Ejes de vinculacion
            ...
            $table->foreignId('assigned_line_id')->constrained('app.catalogues');//pendiente//lineas de investigacion
            $table->string('code',100);
            $table->text('name',500);
            $table->foreignId('status_id')->constrained('app.catalogues');//catalogo propio una fk
            $table->boolean('state')->default(true);
            $table->string('field',100);//campo de area de vinculacion
            $table->string('aim',100);//objeto
            $table->foreignId('fraquency_id')->constrained('app.catalogues');//frecuencia de actividades
            $table->json('cycle')->nullable();//ciclo
            $table->foreignId('location_id')->nullable()->constrained('app.catalogues');//crear tabla de localizacion fk
            ...
            //$table->string('schedules')->constrained('app.files');//cronograma
